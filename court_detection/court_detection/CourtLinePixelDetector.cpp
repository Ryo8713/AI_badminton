...
{
	Mat imgYCbCr;
	cvtColor(frame, imgYCbCr, COLOR_RGB2YCrCb); // modified (opencv 版本差異)
	...
	for (int x = 0; x < binaryImage.cols; ++x)
	{
		for (int y = 0; y < binaryImage.rows; ++y)
		{
			...
			if (value == GlobalParameters().fgValue)
			{
				...
				//// 輸出 Mat l 的內容
				//std::cout << "Eigenvalues at (" << y << ", " << x << "):" << std::endl;
				//for (int i = 0; i < l.rows; ++i)
				//{
				//	for (int j = 0; j < l.cols; ++j)
				//	{
				//		std::cout << l.at<float>(i, j) << " ";
				//	}
				//	std::cout << std::endl;
				//}

				if (l.at<float>(0, 0) > 4 * l.at<float>(1, 0)) // modified
				{
