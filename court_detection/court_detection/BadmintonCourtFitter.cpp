	...
	const Mat& rgbImage)
{
	...
	const int timeLimit = 60; // Add
	int elapsedTime; // Add
	const int minThresh = 2 * 1600, goodThresh = 2 * 2100; // default
	if (bestScore < minThresh) {
		...
		for (int t = 0; t < trials; t++) {
			...
			if ((elapsedTime = TimeMeasurement::stop("LineDetection")) > timeLimit) { // Add
				std::cerr << "Timeout. (elapsed time " << elapsedTime << "s > time limit " << timeLimit << "s)" << std::endl;
				exit(4);
			}
		}
