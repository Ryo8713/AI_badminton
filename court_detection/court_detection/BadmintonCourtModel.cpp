...
{
	//assert(o.netPoints.size() == 2); // µù¸Ñ±¼
	transformationMatrix = o.transformationMatrix;
	netPoints = o.netPoints;
	return *this;
}
...
void BadmintonCourtModel::writeToFile(const std::string& filename)
{
	...
	std::vector<Point2f> corner = {
		transformedModelPoints[0],  // Top-left
		transformedModelPoints[1],  // Bottom-left
		transformedModelPoints[2],  // Bottom-right
		transformedModelPoints[3],  // Top-right
	};
	...
	std::vector<Point2f> realPoints = {
		Point2f(0, 0),    // Top-left corner (0, 0)
		Point2f(0, 13.41),  // Bottom-left corner (0, 13.4)
		Point2f(6.1, 13.41), // Bottom-right corner (6.1, 13.4)
		Point2f(6.1, 0),  // Top-right corner (6.1, 0)
	};

	Mat homographyMatrix = findHomography(corner, realPoints);


	if (homographyMatrix.empty()) {
		throw std::runtime_error("Homography calculation failed.");
	}

	std::vector<Point2f> transformedCorners;
	perspectiveTransform(realPoints, transformedCorners, homographyMatrix);

	for (const auto& pt : transformedCorners) {
		std::cout << "Transformed corner: " << pt.x << ", " << pt.y << std::endl;
	}

	outFile << "Homography Matrix:" << std::endl << homographyMatrix << std::endl;
