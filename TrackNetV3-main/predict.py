...
if __name__ == "__main__":  # add
    ...
    parser.add_argument('--verbose', action='store_true', default=False, help='whether to output the detected frame numbers')
    ...
    save_dir = os.path.abspath(args.save_dir)  # Ensure absolute path
    video_name = os.path.splitext(os.path.basename(video_file))[0]  # Extract filename without extension
    video_format = video_file.split('.')[-1]

    #video_name = video_file.split('/')[-1][:-4]
    #video_format = video_file.split('/')[-1][-3:]
    out_video_file = os.path.join(save_dir, f"{video_name}_pred.{video_format}")
